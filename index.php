                                ...
                                if ($mainInvoice) {
                                    ...
                                    foreach ($tasks as $task) {
                                        ...
                                        if (!isset($includedRental[$task['removal']['rental']['id']])) {
                                            ...
                                            $initialDate = Carbon::parse($task['removal']['rental']['delivery_date'])->addDay()->format('Y-m-d H:i:s');

                                            if (InvoiceItem::whereHas('task.removal.rental', function ($inB) use ($task) {
                                                $inB->where('id', $task['removal']['rental']['id']);
                                            })->exists()) {
                                                $initialDate = InvoiceItem::whereHas('task.removal.rental', function ($inB) use ($task) {
                                                    $inB->where('id', $task['removal']['rental']['id']);
                                                })
                                                ...
                                                $initialDate = Carbon::parse($initialDate)->addDay()->format('Y-m-d H:i:s');
                                            }
                                            ...
                                            $totalDaysToCount = Carbon::parse($myFromDate)->diffInDays(Carbon::parse($myToDate)) + 1;
                        ...
                        if (isset($dates[0]) && isset($dates[1])) {
                            ...
                            if (!empty($tasks) && InvoiceItem::whereIn('task_id', $tasksToBeConsidered)->doesntExist()) {
                                ...
                                foreach ($tasks as $task) {
                                    ...
                                    $finalTotalForTax = $thisRentalRate = $newRentalToBeAdd = 0;
                                    ...
                                    if (isset($task['removal']['rental']['id'])) {
                                        ...
                                        if (!isset($includedRental[$task['removal']['rental']['id']])) {
                                            ...
                                            if (isset($allData[$customer->id])) {
                                                $allData[$customer->id]['total'] += $thisRentalRate + $newRentalToBeAdd + $finalTotalForTax;
                                            } else {
                                    ...
                                    if (isset($allData[$customer->id])) {
                                        $allData[$customer->id]['total'] += isset($task['removal']) ? $task['removal']['total'] : 0;
                                    } else {
                                        $allData[$customer->id]['checkbox'] = "<input type='checkbox' class='check-user' value='$customer->id' />";
                                        $allData[$customer->id]['name'] = $customer->name;
                                        $allData[$customer->id]['total'] = isset($task['removal']) ? $task['removal']['total'] : 0;
