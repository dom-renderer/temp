...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('notification-templates.update', encrypt($template->id)) }}" method="POST" id="notificationTemplateForm">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="type[]" id="type" class="form-select select2" multiple required>
                                    @foreach($notificationTypes as $typeSlug => $type)
                                        <option value="{{ $typeSlug }}" {{ in_array($typeSlug, $template->type) ? 'selected' : '' }}>{{ $type }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="body_editor" style="height: 250px;">{!! old('long_desc', $template->body) !!}</div>
                    </div>
