...
class NotificationHourBeforeStart extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'hour:before';
    ...
    public function handle()
    {
        ...
        foreach ($tasks as $task) {
            ...
            if (!empty($startAt)) {
                $startTimeBeforeHour = Carbon::createFromFormat('Y-m-d H:i:s', date('Y-m-d', strtotime($task->date)) . ' ' . date('H:i:s', strtotime($startAt)));

                $now = Carbon::now();
                $windowStart = $startTimeBeforeHour->copy()->subHour()->subMinutes(2);
                $windowEnd = $startTimeBeforeHour->copy()->subHour();
