...
class GenerateRemovals implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($rental) {
            ...
            if ($rental->frequency_contract != 1) {
                ...
                try {
                    if (!empty($removalDates)) {
                        ...
                        foreach ($removalDates as $key => $removalDate) {
                            ...
                            if ($key == 0) {
                                if (Removal::where('rental_id', $rental->id)->where('removal_type', 4)->whereNotIn('status', [0, 2])->exists()) {
                                    continue;
                                }

                                $subTotal = $rental->total_rental_rate;
                            } else if ($key == count($removalDates) - 1) {
                                if ($rental->perpatual) {
                                    $subTotal = $rental->removal;
                                } else {
                                    // $subTotal = $rental->removal + $rental->final_removal;
                                    $subTotal = $rental->removal;
                                }
                            } else {
                                $subTotal = $rental->removal;
                            ...
                            if (is_numeric($rental->tax) && $rental->tax > 0) {
                                $theTax = Tax::find($rental->tax);
                                if ($theTax) {
                                    $subTotalTax = ($subTotal * $theTax->rate) / 100;
                                }
                            }

                            $grandTotal = $subTotal + $subTotalTax;
                            $removalTypeDefiner = 4;

                            if ($key == 1) {
                                if (count($removalDates) == 2) {
                                    if ($rental->perpatual == 1) {
                                        $removalTypeDefiner = 0;
                                    } else {
                                        $removalTypeDefiner = 3;
                                        if (Removal::where('rental_id', $rental->id)->whereIn('removal_type', [1, 3])->whereNotIn('status', [0, 2])->exists()) {
                                            continue;
                                        }
                                    }
                                } else {
                                    if ($rental->perpatual == 1) {
                                        ...
                                        $removalTypeDefiner = 1;
                                        if (Removal::where('rental_id', $rental->id)->whereIn('removal_type', [1, 3])->whereNotIn('status', [0, 2])->exists()) {
                                            continue;
                                        }
                                    }
                                }
                            } else if ($key == (count($removalDates) - 1) && $key != 0) {
                                    if ($rental->perpatual == 1) {
                                        $removalTypeDefiner = 0;
                                    } else {
                                        $removalTypeDefiner = 2;
                                    }
                            } else if ($key == 0) {
                                $removalTypeDefiner = 4;
                            } else {
                            ...
                            $thisRemoval = Removal::create([
                                ...
                                'type' => $key == 0 ? 1 : 0,
                                'removal_type' => $removalTypeDefiner,
                                'from_date' => $fromDate,
                                'to_date' => $toDate,
                            ...
                            $task = Task::create([
                                ...
                                'job_type' => $thisRemoval->removal_type == 4 ? 0 : (in_array($thisRemoval->removal_type, [0, 1]) ? 1 : 2),
                ...
                $removalDates = collect($removalDates)->prepend(date('Y-m-d H:i:s', strtotime($firstDeliverDate)))->values()->toArray();
